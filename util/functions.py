...
def get_phonemes(pred_y,true_y,data):
    ed_accumalate = []
    for p,t in zip(pred_y,true_y):
        compressed_t = [w for w in t if (w!=1 and w!=0)]
        ...
        if data == 'timit':
            compressed_t = collapse_phn(compressed_t)
            compressed_p = collapse_phn(compressed_p)
    return compressed_p, compressed_t
...
def test_file(batch_data, batch_label, listener, speller, optimizer, data='timit',**kwargs):
    bucketing = kwargs['bucketing']
    use_gpu = kwargs['use_gpu']
    output_class_dim = kwargs['output_class_dim']
    label_smoothing = kwargs['label_smoothing']
    ...
    return get_phonemes(torch.max(pred_y.permute(0,2,1),dim=2)[1].cpu().numpy()
                        ,true_y.cpu().data.numpy(),data)
...
def collapse_phn(seq, return_phn = True, drop_q = True):
